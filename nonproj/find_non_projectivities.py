...
def get_gap_degree(relationships_dict, recursive_children_dict):
	...
	for node in recursive_children_dict:
		...
		if recursive_children_dict[node] is not None:
			...
			for check_node in range(recursive_children_dict[node][0], recursive_children_dict[node][
				-1]):  # we can always be sure that i, j will be in get_nodes_in_subtree for sure
...
if __name__ == "__main__":
	...
	group0.add_argument("-i", "--input", nargs="+",
						help="Input files to read data from, in CONLL-U format. Multiple values possible.")
	group0.add_argument("-id", "--input_directory", type=str,
						help="Parent directory to read Conll-U files from. Reads .conllu files recursively.")
	group2 = parser.add_mutually_exclusive_group(required=False)
	group2.add_argument("--non_planar", action="store_true",
						help="Display the filename,SentID and edges which contribute to NON-PLANAR STRUCTURES in a non-projective tree")
	group2.add_argument("--ill_nested", action="store_true",
						help="Display the filename,SentID and edges which contribute to ILL-NESTED STRUCTURES in a non-projective tree")
	...
	for i in input_files:
		with open(i, "r", encoding="utf-8") as infile:
			...
			if not (args.non_planar or args.ill_nested):
				print("--all", i.split("/")[-1], str(non_proj_edges), str(total_edges), str(non_proj_trees),
					  str(total_trees), sep="\t")
